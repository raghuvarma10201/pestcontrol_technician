...
const CustomerSignature = forwardRef(({ setCustomerSignature }: Technician, ref) => {
  const [sign, setSign] = useState<string | null>(null); // Initialize with null
  ...
  const handleGenerate = () => {
    ...
    if (teachgeneratedUrl) {
      ...
      setCustomerSignature(teachgeneratedUrl);
      setIsEditable(false);
      setErrorMessage(null);
      setButtonText("Saved");
    }
  ...
  return (
    <IonItemGroup className="sigIonGroup">
      <IonLabel className="ion-label">
        Customer Signature <IonText>*</IonText>
      </IonLabel>
      <IonItem>
        <SignatureCanvas
          canvasProps={{
