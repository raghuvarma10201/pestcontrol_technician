...
const Site: React.FC = () => {
  ...
  const handleTaskClick = async (
    ...
    console.log("Site: Task clicked. Task ID = ", taskId);
    // routerLink={`/siteviewLocation/${task.id}`}
    history.push(`/siteviewLocation/${taskId}`);
  };
  ...
  return (
    <IonPage>
      <CommonHeader backToPath={"/dashboard"} pageTitle={"Site"} />
      <IonContent
        fullscreen
                ...
                filteredTasks.map((task: any, index: any) => (
                  ...
                  >
                    <TaskComponent
                      ...
                      date={`${formatDate(task.created_on)}  ${formatTime(task.created_on)}`}
